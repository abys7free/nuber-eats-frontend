...
export const LoggedInRouter = () => {
  ...
  return (
    <Router>
      <Header />
      <Switch>
        {data.me.role === UserRole.Owner && restaurantRoutes.map(route => <Route exact key={route.path} path={route.path}>{route.component}</Route>)}
        ...
        {commonRoutes.map(route => <Route exact key={route.path} path={route.path}>{route.component}</Route>)}
        <Route>
          <NotFound />
        </Route>
        {/* <Redirect to="/" /> */}
